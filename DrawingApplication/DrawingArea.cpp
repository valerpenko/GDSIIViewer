...
void DrawingArea::paintEvent(QPaintEvent *event)
{
    ...
    //qDebug()<<"X/2 inv ="<<-GlobalWidth/(2*winToProjCoef);
    //qDebug()<<"Y/2 inv ="<<-GlobalHeight/(2*winToProjCoef);
    //QTransform moveTo0_0(1,0,0,1,-GlobalWidth/(2),-GlobalHeight/2);
    //QTransform moveToCenter(1,0,0,1,GlobalWidth/2,GlobalHeight/2);
    //QTransform axisYMat(1,0,0,1,0,GlobalHeight);//height());
    //result=axisYMat;//moveTo0_0*scrollMat*moveToCenter;//axisYMat*moveTo0_0*scaleMat*moveToCenter*scrollMat;
    QSize widgetSize=this->size();
    //QTransform moveTo0_0(1,0,0,1,-widgetSize.width()/2,-widgetSize.height()/2);
    //QTransform moveToCenter(1,0,0,1,widgetSize.width()/2,widgetSize.height()/2);
    //QTransform moveToCenter=GlobalWidth>GlobalHeight? QTransform(1,0,0,1,0,widgetSize.height()/2):QTransform(1,0,0,1,widgetSize.width()/2,0);
    //QTransform axisYMat(1,0,0,1,0,height());
    //QTransform moveTo0_0(1,0,0,1,-GlobalWidth/(2*winToProjCoef),GlobalHeight/(2*winToProjCoef));
    ...
    QTransform scaleMat(scaleValue,0,0,scaleValue,0,0);//вставить умножение на коеф проект-окно
    QTransform coefMat(1/winToProjCoef,0,0,1/winToProjCoef,0,0);
    QTransform result;
    result=axisYMat*moveTo0_0_Pr*scaleMat*moveToCenter_Pr*scrollMat*coefMat*moveToCenter;//scrollMat;//*scrollMat;//axisYMat*moveTo0_0*scaleMat*moveToCenter*scrollMat;//
...
void DrawingArea::DrawNet(QPainter* painter){
    ...
    pen.setWidth(winToProjCoef>=1?5*winToProjCoef/scaleValue:winToProjCoef*5);
    pen.setColor(Qt::red);
    painter->setPen(pen);
...
void DrawingArea::SetLineContainer(LineContainer *cont){
    ...
    int bottomX=container.GetBottomX()>0?-container.GetBottomX():container.GetBottomX();
    int bottomY=container.GetBottomY()>0?-container.GetBottomY():container.GetBottomY();
    for(int i=0;i<container.GetAmount();i++)
    {
        //bad solution
        ...
        qDebug()<<"["<<i<<"] Line:["<<lines[i].p1().x()<<","<<lines[i].p1().y()<<"] & ["<<lines[i].p2().x()<<","<<lines[i].p2().y()<<"]";
    }
}
